 ...
 */
async function runXCTestViaRemoteXPC(udid, testRunnerBundleId, appUnderTestBundleId, xctestBundleId, testType = 'ui', args = [], env, timeout = 360000) {
    // Logic tests don't use testmanagerd — skip RemoteXPC
    if (testType === 'logic') {
        throw new Error('Logic tests are not supported via RemoteXPC');
    }
    const XCTestRunnerClass = await (0, remotexpc_utils_1.getXCTestRunnerClass)();
    const runnerOptions = {
        udid,
        testRunnerBundleId,
 ...
 */
async function listXCTestBundlesViaRemoteXPC(udid) {
    const installProxy = await installation_proxy_client_1.InstallationProxyClient.create(udid, true);
    try {
 ...
 */
async function installXCTestBundleViaRemoteXPC(udid, xctestApp) {
